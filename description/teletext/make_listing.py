...
def make_text(page, local_links = False):
    ...
    text += re.sub(r"([^\s\>])?P(\d\d\d)", rf'<a href="{local_links and "#P" or ""}\2{not local_links and ".html" or ""}">\1P\2</a>', body, re.IGNORECASE)
    text = re.sub(r"(http(s)?://[^\s,\<]*)", r'<a href="\1" target="_blank">\1</a>', text, re.IGNORECASE)
