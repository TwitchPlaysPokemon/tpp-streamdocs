...
def make_text(page, local_links = False):
    ...
    text += re.sub(r"([^\s\>])?(P)(\d\d\d)", rf'<a href="{local_links and "#P" or ""}\3{not local_links and ".html" or ""}">\1\2\3</a>', body)
...
disclaimer = "<!-- This page is autogenerated by make_listing.py. Do not edit this page. Edit the teletext page(s), page.template.html, or make_listing.py instead. -->\n\n\n"
with open(os.path.join(script_dir, "page.template.html"), "rb") as f:
    # Now 24 lines of 40 characters follow (the header is omitted)
...
for p in range(len(pages)):
    ...
    html = disclaimer + template.replace("--PAGE--", img).replace("--TITLE--", title).replace("--NUMBER--", str(page["number"])).replace("--VALIDPAGES--", str.join(",", [str(page["number"]) for page in pages]))
